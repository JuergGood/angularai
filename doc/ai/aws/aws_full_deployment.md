...
aws ecs update-service --cluster angular-boot --service angularai-backend-test-service --task-definition angularai-backend:5 --query "service.taskDefinition" --output text
```
...
```bash
# 2. Update service (Replace REVISION with the number from step 1)
aws ecs update-service --cluster angular-boot --service angularai-frontend-service --task-definition angularai-frontend:3 --query "service.taskDefinition" --output text
```
...
- **Expected Mode**: `Postgres`

### 6.2 Troubleshooting "Old Version Still Displayed"
If you still see `0.0.1-SNAPSHOT` after a successful deployment:

1.  **Check ECR Timestamps**: Go to the AWS Console -> ECR -> `angularai-backend` repository. Check the "Pushed at" time for the `latest` tag and the `1.0.1` tag. If they are old, your `docker push` didn't work.
2.  **Verify Local Tagging**: Run `docker images` and ensure that `angularai-backend:1.0.1` and the ECR-tagged version exist and have a recent "CREATED" time.
...
4.  **Clean Local Build**: Sometimes Docker caches old layers. Try building with `--no-cache`:
    ...
    ```
5.  **Force ECS Pull**: When using the `latest` tag, ECS sometimes doesn't pull the new image if the tag hasn't changed. The `aws ecs update-service --force-new-deployment` command *should* fix this, but using an explicit version (e.g., `:1.0.1`) in your Task Definition is **much safer**.
